...
class TacticalDrawingTab(QWidget):
    ...
    def start_path_editing(self):
        ...
        self.original_path_data = path_data.copy()
    ...
    def save_path_edits(self):
        ...
        for i in range(self.path_list.count()):
            ...
            if item.text().startswith(self.editing_path_id):
                color = "green" if path_data['type'] == 'aircraft' else "red"
                item.setText(
    ...
    def cancel_path_editing(self):
        """Cancel editing and revert changes (NEW)"""
        if not self.editing_path_id or not self.original_path_data:
            return
        
        # Restore original path data
        self.path_manager.update_path_points(
            self.editing_path_id, 
            self.original_path_data['points']
        )
        
        # Emit signal to JavaScript
        self.bridge.stopPathEditing.emit()
        
        # Send updated path to map
        path_data_json = json.dumps(self.original_path_data)
    ...
    def delete_selected_path(self):
        ...
        # Delete from PathManager
        deleted = self.path_manager.delete_path(path_id)
        if not deleted:
            QMessageBox.warning(self, "Error", "Failed to delete path.")
            return
        
        # Remove from list
        for i in range(self.path_list.count()):
            item = self.path_list.item(i)
            if item.text().startswith(path_id):
    ...
    def on_waypoint_moved(self, update_json):
        ...
        try:
            ...
            path_data = self.path_manager.get_path_by_id(path_id)
            if path_data and 0 <= index < len(path_data['points']):
                path_data['points'][index] = [lat, lon]
                self.path_manager.update_path_points(path_id, path_data['points'])
    ...
    def on_waypoint_inserted(self, insert_json):
        ...
        try:
            ...
            # Insert waypoint in PathManager
    ...
    def on_waypoint_deleted(self, delete_json):
        ...
        try:
            ...
            # Remove waypoint from PathManager
    ...
    def on_path_drawn(self, path_json):
        ...
        try:
            ...
            # Add to list
            self.path_list.addItem(
                f"{path_id} | {distance:.1f}nm | {path_data['speed_kts']}kts | {color}"
            )
